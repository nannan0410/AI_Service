...
/** 寒暄 / 闲聊短句（无业务关键词时跳过 LLM 语义路由与主模型） */
const CHITCHAT_PATTERN =
  /^(hi|hello|hey|yo|ok|okay|thanks|thank you|哈喽|你好|您好|在吗|在么|在不在|早上好|下午好|晚上好|谢谢|感谢|多谢|好的|好哒|嗯|哦|啊|哈+|嗨|喂|拜拜|再见|bye|test|测试)[!！?？~～\s]*$/i

/**
 * 判断是否像纯闲聊（无落地 Workflow / 业务 Tool 需求）。
 * 用于跳过 LLM Skill 分类，并本地秒回（不调主模型）。
 */
export function isLikelyGeneralMessage(message: string): boolean {
  ...
  // 仅标点 / 表情等无意义短串
  if (text.length <= 8 && !/[\u4e00-\u9fffA-Za-z0-9]/.test(text)) return true
  return false
}
