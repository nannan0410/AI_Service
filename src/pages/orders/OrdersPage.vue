...
function filterByTab(tab: number) {
  ...
  if (tab === 1) return orders.value.filter((o) => o.status === 'paid' || o.status === 'completed')
...
<template>
  <div class="page">
    ...
    <van-tabs v-model:active="activeTab">
      <van-tab title="待支付">
        <van-empty v-if="filterByTab(0).length === 0" description="暂无待支付订单" />
      ...
      <van-tab title="已退款">
        <van-empty v-if="filterByTab(2).length === 0" description="暂无退款订单" />
        <van-cell-group v-else inset class="list">
          <van-cell
