...
async function onPay() {
  ...
  loading.value = true;
  try {
    const { data: res } = await payParking({
      plateNo: plateNo.value,
      amount: parkingInfo.value.amount,
    });
    if (res.code === 200) {
      showToast("支付成功");
      mode.value = "select";
...
<template>
  <div class="page">
    ...
    <div v-else-if="mode === 'bind'" class="content">
      ...
      <div class="actions">
        <van-button block type="primary" :loading="loading" @click="onBind"
          >确认绑定</van-button
        >
    ...
    <div v-else class="content">
      <van-cell-group inset>
        ...
        <van-cell
          title="应付金额"
          :value="parkingInfo ? `¥${parkingInfo.amount}` : '-'"
        />
      </van-cell-group>
      <div class="actions">
        <van-button block type="primary" :loading="loading" @click="onPay"
          >确认支付</van-button
        >
        <van-button block plain @click="backToSelect">返回</van-button>
      </div>
