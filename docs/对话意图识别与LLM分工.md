...
|------|------|------------|----------------------|
| **① 标签类** | 读画像标签做排序/选品，并可展示「因标签推荐」 | 订单标签、会员标签、事实/消费标签；对话推断仅**会话内**（`UserProfileTags.aiTags` / `prefer_*`，**不落库、不回写**会员；正式对话打标为二期 / 三期） | `因亲子标签推荐`、会员选品角标中的「· 亲子标签」等 |
...
窗口非实名购票、他人代买未归集到账号订单、又未授权定位时，无法用「当日票/单」或 GPS 推断是否在园，欢迎区推荐易停在游前集合。产品侧用自报兜底补一层。

**落地触发条件（三条件同时满足才展示询问）**：

| # | 条件 | 说明 |
|---|------|------|
| 1 | **没有当日订单**（含 OTA/TA） | 当前景区、出行日=今天；订单侧查不到与本人相关的当日单 |
| 2 | **票夹没有当日票** | 按当前账号绑定实名游客倒查票务；含他人下单但票面含本人的当日票；票夹当日为空 |
| 3 | **未授权定位** | 无法用园区围栏补 `inPark` |

交互：友好询问是否在园（文案示例：「您现在在园里吗？点一下状态，推荐会更准～」）；选「在园」后按在园态刷新推荐入口（**不**伪造订单/票夹标签）；选「未到园」则保持游前推荐。与 `inPark` 正式合成规则（订单已核销 ∨ 票夹已核销 ∨ GPS）互补：上述三路都推不出「今日有票/可定位」时才问。字段细节见 [`业务场景配置与实现.md`](./业务场景配置与实现.md) §2.4.2 / §2.4.3。

**落地·手动标记清空与频次（同日最多问一次）**：

| 项 | 口径 |
|----|------|
| 存储维度 | **账号 + 当前景区**；记录 `lastSelfReportDate`（景区时区自然日）与自报结果 |
| 跨日清空 | 今天 ≠ `lastSelfReportDate` → **清除**手动标记，允许再次询问（「是否在园」是当日态，与当日订单/票夹条件一致） |
| 同日 | 无论选在园 / 未到园，**当天不再询问**（同日最多 1 次，不显得过频） |
| 与系统信号 | 一旦出现当日订单 / 票夹当日票 / 定位可判在园 → **以系统信号为准**，可覆盖或忽略自报；三条件不再满足时也不再展示询问 |

### 1.4 购票·儿童身高 / 老人年龄资格（已实现）

| 项 | 口径 |
|----|------|
| 触发 | 人数齐且已有出行日期、出推荐票卡前；`child>0` 问身高，`elderly>0` 问年龄（可先后两张卡） |
| 问法 | **整批一次是/否**（对话卡点选；也可打字「是/否」） |
| 阈值 | **按景区可配**：`childMaxHeightCm`（默认 120，身高 **≤** 可买儿童票）、`elderMinAgeYears`（默认 60）；Mock `ticket_eligibility.json` + 业务场景页本机覆盖 |
| 儿童「否」 | 文案「由于身高超过儿童购票限制，推荐购买成人票」；儿童人数并入成人再匹配（**含家庭套票**，如 2大1小→按 3 成人） |
| 老人「否」 | 文案「未满年龄要求，推荐购买成人票」；老人人数并入成人再匹配 |
| 老人「是」 | 保留老人位，匹配老人票 SKU（演示 `tkt_elderly`）；无 SKU 则兜底购票列表 |
| 改人数 | 清空已答资格，按新人数重问 |
| 代码 | `ticketEligibility.ts`、`purchaseStore` 步 `ask_child_height` / `ask_elder_age`、`TicketEligibilityCard`、`ticketPurchase.ts` |
...
| **购票·资格确认** | 含儿童/老人时出票前是/否卡 | `ticketEligibility.ts` + `TicketEligibilityCard` | ❌ | **保持规则**；阈值景区可配 |
...
| **服务点评（入口）** | Skill + 点评正则 | `reviewService.ts` → ReviewCard（景区日评，去订单） | ❌ | **保持规则**；见 [`景区点评优化规格.md`](./景区点评优化规格.md) |
| **服务点评·帮我写评价** | 卡片/假页按钮 | `generateReviewDraft.ts` | ⚡ 混合 | LLM≈50 字；无 Key 离线模板；**不自动提交** |
| **服务点评·分享发券** | 提交后四渠道 Mock | `POST /api/reviews/share` | ❌ | 须 >20 字+≥2 图；日限发券 1 次 |
...
| 日期 | 说明 |
|------|------|
| 2026-08-12 | 景区点评优化定稿：去订单、准入 OR、日限、分享后发券、点评管理；见 `景区点评优化规格.md` |
| 2026-08-12 | §1.4 购票儿童身高/老人年龄资格确认已实现（整批是/否卡；阈值可配；超限并入成人再匹配） |
| 2026-08-12 | §1.3 在园自报：落地三条件（无当日订单∧票夹无当日票∧未授权定位）；**跨日清空、同日最多问一次**；不写演示触发细节 |
