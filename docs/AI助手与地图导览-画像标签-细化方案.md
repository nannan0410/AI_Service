...
| P2-B3 | AI 推断 | **演示**：固定词「刺激 / 拍照\|出片 / 休闲」→ 会话内 `prefer_*`，**不回写会员**。正式喜好目录与勾选在**会员系统**；助手只读对照 |
...
| 域 | 后台维护 | 写入时机 | 说明 |
|----|----------|----------|------|
| **订单标签** | 名称（随预置模板）+ **固定规则模板**（仅可配阈值 / 启停） | 助手**读订单时现算**（含历史单补打） | 如亲子/情侣/团队；**不可空手新建订单标签** |
| **会员标签** | 名称（**规则 / 手动**）+ 规则类阈值 | 规则现算或定时；手动运营打 | **喜好不在本页维护**（见下）。后台新建仅手动；规则类由模板写入 |
| **产品标签** | 名称目录 | 产品管理勾选 | 与订单/会员分域 |
| **项目 / 内容标签** | 一期不做 | — | 待未来「项目列表」主数据再加 |

**喜好（会员系统，非 AI 配置）：** 喜好是会员参数的一类，目录与游客勾选都在会员系统。AI 标签页**不可新建喜好**，只读展示对照（会员参数 code → 助手内部信号）。对话推断喜好仅会话内加权，**不回写**会员系统。
...
flowchart LR
  ...
  FactRule & ManualTag --> Center["会员画像快照"]
  MemberPref --> Center
  OrderTags --> Assistant["AI 助手只读"]
  Center --> Assistant
...
1. **标签目录**模块：能按**订单 / 会员 / 产品**分域看清；会员区分规则/手动；喜好为会员系统只读对照。  
