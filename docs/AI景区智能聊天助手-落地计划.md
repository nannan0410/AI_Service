...
| 购票 | `ticket_purchase` | 逐步确认人数/日期/票种 → 选游客 → 推券并默认带上 → OrderCard → **跳 H5 提交订单假页** → 模拟支付 → 回聊天 |
...
用户：「两大一小，有优惠吗」
  ...
  → 选 commonVisitors
  → 提示“有个小惊喜”
  → getCoupons + 发券到账号 + 默认带上优惠券
  → createOrderDraft → OrderCard
  → 用户点「去提交订单」
  → navigate /order/submit?draftId=xxx（H5 假页）
  → 假页：条款勾选 + Mock 支付
  → 返回 /chat：system「支付成功」+ 助手 nod
  → 推送游玩攻略：交通 + 入园 + 推荐项目 + 保存攻略图
```

**推荐规则示例**：`新客` → 推新客券；`亲子` → 推家庭套票；`高价值` → 推会员专享。

**购票话术要求**：助手像朋友聊天，回复短句优先，每次尽量不超过 60 字。信息缺失时一次只追问一个关键项；关键问题用 `**关键词**` 标记以便聊天气泡高亮。推荐和带券时多使用「好眼光」「真棒」「我帮您看好了」「这个很适合」「好嘞」「收到」「玩得开心」等正向反馈。下单只说 H5 确认下单/模拟支付，不说打开 App。

**动态快捷按钮**：助手回复后按当前步骤展示最相关动作。优惠券阶段展示「用券买票 / 看我的券」；日期阶段展示「今天 / 明天 / 本周六 / 本周日」；人数阶段展示「1位 / 2位 / 3位」；票种阶段展示「选这张」；路线攻略阶段展示「订票走起 / 看实时排队」。确认下单弹窗打开时不展示快捷按钮。
...
| v0.6.2 | 2026-06-16 | 完善购票意图识别后的确认信息、票种推荐、选游客、带券下单和正向话术要求 |
